// Alumno: ALBERT MATOS
// Matricula: 10163294
// Materia: Laboratorio Microcontroladores Aplicados (IMC 1910)
...
// Configuracion de Salidas de Motor (Relays)
sbit MOTOR_DER at LATB.B6;
sbit MOTOR_IZQ at LATB.B7;

// COonfiguracion del teclado
    ...
    if (tecla_nueva >= '0' && tecla_nueva <= '9') {
        ...
        if (indice_teclas == 4) {
            ...
            if (buffer_teclas[0]=='8' && buffer_teclas[1]=='0' && buffer_teclas[2]=='0' && buffer_teclas[3]=='8') {
                ...
                Lcd_Out(2, 1, "Espere...      ");
                Delay_ms(2000);
                estado = 2; //Estado para menu de ordenes
            } else {
...
void Control_Motor(char dir) {
    // Iseguridad para que no encienda el motor
    MOTOR_DER = 0;
    MOTOR_IZQ = 0;
    ...
    // Solo cuando se pulsa # entramos al bucle de movimiento
    if (tecla_nueva == '#') {
        Lcd_Cmd(_LCD_CLEAR);

        while(1) {
            // para detectar el paro
            ...
            //Si se pulsa # de nuevo, apagamos y salimos
            if (tecla_nueva == '#') {
                MOTOR_DER = 0;
                MOTOR_IZQ = 0;
                Lcd_Cmd(_LCD_CLEAR);
...
void main() {
    ...
    while(1) {
        ...
        switch (estado) {
            ...
            case 3: Control_Motor(1); break; // Caso Derecha
            case 4: Control_Motor(2); break; // Caso Izquierda
        }
        Delay_ms(20);
    }
}